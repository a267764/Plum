...
object ApiJinRiShiCi {
    ...
    val keySentenceByToken: String
        get() {
            val conn = Jsoup.connect("https://v2.jinrishici.com/sentence")
            conn.header("User-Agent", NetworkUtil.userAgent)
            val token: String = PlumConfig.functions.dailyPoetry.jinRiShiCi.token
            conn.header("X-User-Token", token)
            val responseJson: Connection.Response = try {
            ...
            /** 进行JSON解析  */
            val resultJson = responseJson.body()
            val jo = JsonParser.parseString(resultJson) as JsonObject
            val result = jo["data"].asJsonObject["content"].asString

            /** 判断warning是否为null  */
            val warning: String
            val je = jo["warning"]
            warning = if (je is JsonNull) "NO WARNING." else je.asString
